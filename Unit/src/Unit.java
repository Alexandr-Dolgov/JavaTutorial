public class Unit {
    ...
    public void go(Point point) {//принимает координату, вычисляет расстояние до новой координаты точки
        ...
        System.out.println("при перемещении в точку " + "(" + point.x +":"+ point.y+ ")" + " пройдено расстояние " + r + " за  " + quantityHourGo);
    }

    public void goAngleTurn(double angleTurn, double quantityHourGo) {//перемещаем точку на заданное расстояние по направлению, вычисляем координаты точки
        p.x = p.x + speed * quantityHourGo * Math.cos(angleTurn / 360);
        p.y = p.y + speed * quantityHourGo * Math.sin(angleTurn / 360);
        System.out.println("координата новой точки при перемещении в направлении "+ angleTurn +" на "+ speed*quantityHourGo + " за " + quantityHourGo + " (" + p.x + ":" + p.y+ ")");
    }
